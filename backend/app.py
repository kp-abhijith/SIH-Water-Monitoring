...
def receive_data():
    ...
    alarm = 1 if data.get('alarm', False) else 0
    ...
    if alarm == 1:
        alert_msg = f"[SIH ALERT] Water Station {device_id} is discharging UNSAFE water! Outlet TDS: {t_tds}ppm, Turbidity: {t_turb}NTU."
...
def get_history():
    ...
    if limit is not None and 'page' not in request.args:
        cursor.execute(f"SELECT * FROM water_logs {where_clause} ORDER BY timestamp DESC LIMIT ?",
                        params + [limit])
    ...
    cursor.execute(
        f"SELECT * FROM water_logs {where_clause} ORDER BY timestamp DESC LIMIT ? OFFSET ?",
...
def get_latest_status():
    ...
    if device_id:
        cursor.execute(
            "SELECT * FROM water_logs WHERE device_id = ? ORDER BY timestamp DESC LIMIT 1",
            (device_id,)
        )
    else:
        cursor.execute("SELECT * FROM water_logs ORDER BY timestamp DESC LIMIT 1")
...
def list_stations():
    ...
    for s in stations:
        ...
        cursor.execute(
            "SELECT alarm_status FROM water_logs WHERE device_id = ? ORDER BY timestamp DESC LIMIT 1",
            (s["device_id"],)
        )
